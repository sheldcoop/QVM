...
def parse_qvm_content(content: str) -> pd.DataFrame:
    ...
    pad_pattern = re.compile(
        r"Circle:\s+Pad_([A-Z]{2}_\d+)\(ID:\d+,.*?\)\s*"
        r"Coord\.\s*X\s*=\s*([-\d.]+)\s*"
        r"Coord\.\s*Y\s*=\s*([-\d.]+)\s*"
        r"Diameter\s*=\s*([-\d.]+)"
    )
    ...
    via_pattern = re.compile(
        r"Circle:\s+Via_([A-Z]{2}_\d+)\(ID:\d+,.*?\)\s*"
        r"Coord\.\s*X\s*=\s*([-\d.]+)\s*"
        r"Coord\.\s*Y\s*=\s*([-\d.]+)\s*"
        r"Diameter\s*=\s*([-\d.]+)"
    )
    ...
    #           DX =         0.000592
    #           DY =         0.003681
    # Note: the order of Via and Pad in the "between" clause might vary (e.g., Pad... and Via... or Via... and Pad...)
    ...
    for loc, data in parsed_data.items():
        ...
        if 'pad_diameter' not in data or 'via_diameter' not in data or 'sc' not in data:
            # We might just skip incomplete data, but a warning or exception could be raised
            continue
